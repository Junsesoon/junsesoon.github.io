...
function printDryRunSummary(posts: MarkdownPost[], skippedWithoutFrontmatter: number) {
  ...
  console.log('No database writes were performed. Run with --write to import.');
}

async function importPosts(posts: MarkdownPost[]) {
...
async function main() {
  ...
  console.log(`Imported ${posts.length} markdown post(s) into the database.`);
}
