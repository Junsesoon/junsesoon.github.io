...
- `middleware.ts`: 전역 라우팅을 가로채어 관리자 페이지(`/admin/*`) 접근 시 인증 상태(쿠키)를 검증하고, 미인증 접근을 로그인 페이지로 리다이렉트하는 라우트 가드 역할을 수행합니다.

## 게시물 데이터 흐름
- **기존 방식:** `public/posts/...` 폴더 아래의 마크다운 파일들을 렌더링 시점에 파싱하여 페이지로 직접 라우팅했습니다
- **현재 방식:** 마크다운 프론트매터(frontmatter)와 콘텐츠를 먼저 PostgreSQL 데이터베이스로 가져옵니다. 게시물의 모든 동적 속성은 단단하게 고정된 확장 테이블 대신 `posts` 테이블의 단일 JSONB 컬럼(`properties`)으로 유연하게 관리됩니다.
