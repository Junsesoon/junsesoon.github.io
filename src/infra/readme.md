...
- `infra/turso.ts`: Turso DB(SQLite/Libsql) 연결 관리자입니다. `DB_ENV` 환경 변수가 `'local'`일 때는 로컬 SQLite 파일(`file:visitLog.db`)을 사용하도록 우회하며, 원격일 경우 환경 변수(`TURSO_DB_URL`, `TURSO_AUTH_TOKEN`)를 통해 연결 클라이언트를 싱글톤으로 설정하여 제공합니다. 단일 쿼리 실행을 지원하는 `query` 헬퍼 함수를 제공합니다.
