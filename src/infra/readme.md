...
- `infra/init-db.ts`: `src/scripts/` 폴더에 분리된 여러 도메인별 SQL 스크립트 파일(`001_~`, `002_~`, `003_~`)을 순차적으로 실행하여 데이터베이스 스키마와 초기 시드 데이터를 안전하게 초기화합니다. 반복 실행이 가능하도록 멱등성(Idempotency)이 고려되어 있습니다.
