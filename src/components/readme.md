...
- `actions.ts`: 클라이언트 컴포넌트에서 폼 제출 시 호출하는 Next.js Server Actions 모음입니다.
  ...
  - **메타데이터 관리**: 카테고리 템플릿 및 전역 속성을 추가/삭제/이름 변경(Rename)하는 로직을 처리합니다. 특히 속성 이름 변경 시 DB 트랜잭션을 통해 기존 모든 게시물의 JSONB 데이터(`properties`)도 일괄적으로 안전하게 동기화합니다.
...
- `PropertyManager.tsx`: 전역 프론트매터 속성 목록과 사용 횟수를 확인하고, 새로운 속성을 추가하거나 기존 속성을 삭제 및 **이름 수정(Rename)** 할 수 있는 클라이언트 UI 컴포넌트입니다. 이름 수정 시 연관된 모든 게시물 데이터까지 일괄 업데이트됩니다.
